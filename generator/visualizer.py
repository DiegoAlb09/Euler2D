...
def plot_topology_analysis(field, u, v, metrics, save_path, case_name=""):
    ...
    ax2.set_xlabel('Gradiente + Componente Rotacional')
    
    # 3. Análisis de Euler
    ax3 = fig.add_subplot(gs[0, 2])
    euler_data = {
        'Fórmula V-E+F': metrics['euler_vef'],
        'Fórmula β₀-β₁': metrics['euler_poincare']
    }
    bars = ax3.bar(euler_data.keys(), euler_data.values(), 
                   color=['blue', 'orange'], alpha=0.7)
    ax3.set_title('Comparación Fórmulas de Euler')
    ...
    metrics_table = [
        ...
        ['Vértices (V)', f"{metrics['vertices']}", 'Número de vértices en la malla'],
        ['Aristas (E)', f"{metrics['edges']}", 'Número de aristas en la malla'],
        ['Caras (F)', f"{metrics['faces']}", 'Número de caras (píxeles activos)'],
        ...
        ['Consistencia', '✓' if metrics['is_consistent'] else '✗', 'Concordancia entre fórmulas'],
        ['Fracción de Área', f"{metrics['area_fraction']:.3f}", 'Proporción de material']
    ]
    ...
    # Colorear filas de Euler
    for i in [6, 7]:  # Filas de Euler
...
def create_summary_report(cases_data, save_path):
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for case in cases_data:
            ...
            f.write(f"\nConsistencia: {'✓' if m['is_consistent'] else '✗'}\n")
            f.write(f"Fracción de área: {m['area_fraction']:.4f}\n")
            f.write(f"Perímetro: {m['perimeter']}\n")
            f.write("\n" + "="*50 + "\n\n")
